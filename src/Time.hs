...
data Interval =  Secs Rational | Minutes Rational | Hours Rational | Days Rational | Weeks Rational deriving (Show)

intervalToSecs :: Interval -> Rational
intervalToSecs z = case z of (Secs x) -> x
                             (Minutes x) -> 60 * x
                             (Hours x) -> 3600 * x
                             (Days x) -> 24 * 3600 * x
                             (Weeks x) -> 3600 * 24 * 7 * x
...
guardAgainstZero y = case y of (Secs x) -> Secs . filterOutNegative $ x
